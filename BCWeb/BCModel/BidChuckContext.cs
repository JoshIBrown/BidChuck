...
namespace BCModel
{
    public class BidChuckContext : DbContext
    {
        ...
        public BidChuckContext(string currentUser, string ipAdress)
            : base("DefaultConnection")
        {
        ...
        public BidChuckContext(string currentUser, string ipAdress, string connection)
            : base(connection)
        {
            _currentUser = currentUser;
            _ipAddres = ipAdress;
        ...
        protected void ObjectContext_SavingChanges(object sender, EventArgs e)
        {
            ...
            foreach (ObjectStateEntry entry in entries)
            {
                if (!entry.IsRelationship && entry.Entity != null && !(entry.Entity is Audit.DBAudit))
                {
                    ...
                    Audit.DBAudit audit = this.AuditTrailFactory(entry, userName, _ipAddres);
